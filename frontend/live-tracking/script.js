document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadActiveDelivery() {
        try {
            ...
            if (res.ok) {
                ...
                const donations = Object.entries(data || {}).map(([id, val]) => ({id, ...val})).reverse();
                
                // Find a donation that is "In Transit" or "Accepted", or just take the latest
                const active = donations.find(d => d.status === 'In Transit' || d.status === 'Accepted' || d.status === 'Pending AI Match') || donations[0];
                
                if (active) {
                    activeDonationId = active.id;
                    const orderIdEl = document.querySelector('.order-id');
                    if (orderIdEl) {
                        orderIdEl.textContent = `Donation #${active.id.slice(-4)} : ${active.foodItem}`;
                    }

                    // If it's literally Delivered, we skip simulation
        ...
        // Start simulation timer regardless
        setTimeout(() => {
    ...
    async function completeDelivery() {
        ...
        // 🔵 Call the Backend to actually mark this in the Database!
        if (activeDonationId) {
             try {
                 simDeliveredBtn.textContent = 'Updating Database...';
                 await fetch('http://localhost:5000/update-status', {
                     method: 'POST',
                     headers: { 'Content-Type': 'application/json' },
                     body: JSON.stringify({ donationId: activeDonationId, newStatus: 'Delivered' })
                 });
             } catch(e) {
