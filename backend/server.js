...
// Free cloud MongoDB: https://www.mongodb.com/cloud/atlas (create free cluster → get connection string)
// Local MongoDB:      mongodb://127.0.0.1:27017/foodrescue
...
mongoose.connect(MONGO_URI, { serverSelectionTimeoutMS: 5000 })
  ...
  .catch(err => {
    ...
    console.error("👉 Please set MONGO_URI to a valid MongoDB connection string.");
    console.error("   Free cloud DB: https://www.mongodb.com/cloud/atlas");
    // Don't exit — server still starts (useful for health checks)
  });
...
const userSchema = new mongoose.Schema({
  ...
  role:          { type: String, enum: ["donor","restaurant","household","ngo","volunteer","admin","quick_donor"], required: true },
...
const donationSchema = new mongoose.Schema({
  ...
  quantity:   String,
  expiryTime: String,
  status:     { type: String, default: "Pending AI Match", enum: ["Pending AI Match","Accepted","In Transit","Delivered"] },
  session:    String,      // Breakfast | Lunch | Dinner
  donorId:    { type: mongoose.Schema.Types.Mixed }, // ObjectId or 'anonymous'
  donorType:  { type: String, default: "verified" }, // 'verified' | 'quick_donor'
  // Quick donor extra fields
  donorName:  String,
  phone:      String,
  address:    String,
  lat:        Number,
...
const checkDonationTime = (req, res, next) => {
  ...
  else res.status(403).json({ success: false, message: "Food donation is allowed only during Breakfast (7-10 AM), Lunch (12-2 PM), and Dinner (7-10 PM)." });
};
...
app.get("/health", (req, res) =>
  res.json({ status: "ok", db: mongoose.connection.readyState === 1 ? "mongodb" : "disconnected", time: new Date() }));
...
app.post("/register", async (req, res) => {
  try {
    ...
    const user = await User.create({ fullName, email, password: hashedPassword, role, licenseNumber, ngoId });
...
// ══════════════════════════════════════════════════════════════
//  DATA ROUTES (public reads, protected writes)
// ══════════════════════════════════════════════════════════════
...
app.get("/donations", async (req, res) => {
  try {
    ...
    // Support both response shapes for web + mobile
    const mapped = donations.map(d => ({ id: d._id, ...d }));
    res.json({ success: true, donations: mapped });
  } catch (err) {
...
app.post("/donate", optionalAuth, checkDonationTime, async (req, res) => {
  try {
    ...
    res.status(201).json({ success: true, message: "Donation logged successfully.", id: donation._id });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
});

// PATCH update donation status — requires auth
...
// GET notifications — PUBLIC (derived from status changes)
app.get("/notifications", async (req, res) => {
  try {
    const donations = await Donation.find({ status: { $ne: "Pending AI Match" } })
      .sort({ updatedAt: -1 })
      .select("foodItem status updatedAt createdAt")
      .lean();
    const notifications = donations.map(d => ({
      id: d._id, foodItem: d.foodItem, status: d.status,
...
app.listen(PORT, () => {
  ...
  console.log(`🍃 DB: MongoDB at ${MONGO_URI}`);
});
